...
이 경우, 기차 안에 사람이 가장 많은 때는 2번역에서 3명의 사람이 기차에서 내리고, 13명의 사람이 기차에 탔을 때로, 
총 42명의 사람이 기차 안에 있다.
...
int main()
{
    int station[4] = {0,}, m; // 각 역에서의 사람 수, 최대 사람 수
    for(int i=0; i<4; i++){
        int A, B; // 내린 사람 수, 탄 사람 수
        cin >> A >> B;
        station[i] += (B - A);
        if(i+1 != 4)station[i+1] = station[i];
    }
    m = *max_element(station, station + 4);
    cout << m;
    return 0;
}