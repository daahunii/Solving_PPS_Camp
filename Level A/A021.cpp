...
===========================================================================================
입력
첫째 줄에 멀티탭의 개수 N이 주어진다. (1 ≤ N ≤ 500,000) 이어서 
둘째 줄부터 N개의 줄에 걸쳐 각 멀티탭이 몇 개의 플러그를 꽂을 수 있도록 되어 있는지를 나타내는 자연수가 주어진다. 
...
int main()
{
    int multitap, plug_num = 0; // 멀티탭 수, 플러그 수
    cin >> multitap;
    for(int i=0; i<multitap; i++){
        int plug;
        cin >> plug;
        if(i != multitap-1) plug_num += (plug-1); // 마지막 멀티탭 빼고 나머지는 플러그 수에 -1
        else plug_num += plug;
    }
    cout << plug_num;
    return 0;
