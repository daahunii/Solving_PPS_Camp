...
그렇지 않다면, 압축된 결과를 저장할 문자열(result)에 문자열 단위를 붙여주고 문자열 단위를 새로 j번부터 i개로 업데이트한다. 
이때, 문제 조건에 따라서 이 문자열 단위가 여러 개였다면(cnt가 1 이상이라면) cnt를 문자열 앞에 붙여서 result에 추가하고 
cnt는 1로 다시 초기화한다.
================================================================================================
*/
...
int solution(string s) {
    ...
    int half = s.size() / 2;
    for (int i = 1; i <= half; i++){ //i개 단위로 잘라서 압축
        string result = ""; //i개 단위로 잘라서 압축한뒤 만들어지는 문자열
        string sub_s = s.substr(0, i); //자를 문자열 단위
        int cnt = 1;
        
        for(int j = i; j <= s.size(); j += i) { //앞에서 자른 문자열 단위를 제외한 뒷부분(j번 문자부터) 문자열을 검사한다.
            if(sub_s == s.substr(j, i)) cnt += 1;  //j번 부터 i개 만큼이 문자열 단위와 똑같은 경우 cnt 증가
            else{
                if(cnt == 1) result += sub_s; //다른 경우 중 cnt가 1이면 result에 그대로 sub_s를 붙인다.
                else result += (to_string(cnt) + sub_s); //cnt가 1보다 크다면 cnt를 문자열 단위 앞에 붙여서 result에 이어준다.
                
                sub_s = s.substr(j, i); //문자열 단위를 j번부터 i개로 변경 
                cnt = 1; //cnt값 다시 1로 초기화
            }
        }
        if((s.size() % i) != 0) result += s.substr((s.size() / i)*i); //문자열이 i로 나누어 떨어지지 않는다면 result에 나머지 부분을 붙임
        if (answer > result.size()) answer = result.size(); //최솟값을 answer에 저장
        // cout << result << endl;
    }
    return answer;
}
...
int main()
{
    cout << solution("aabbcdaabbcd") << endl;
    return 0;
}